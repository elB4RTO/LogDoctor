...
public:
    ...
    FormatOps::LogsFormat processFormatString(const std::string& formatted_string, const int log_type, const int web_server_id ) const&;
...
private:
   ...
   const  std::vector<std::string> N_ALFs = {"$remote_addr", "$time_local", "$time_iso8601", "$request", "$status", "$bytes_sent", "$request_length", "$request_time", "$http_referer", "$http_user_agent",
                             /*NOT IN USE->*/"$remote_user", "$gzip_ratio", "$connection", "$connection_requests", "$msec", "$pipe"};

    const std::vector<std::string> I_ALFs = {"c-ip", "time", "cs-method", "cs-uri-stem", "sc-status", "sc-bytes", "cs-bytes", "time-taken", "cs(Referrer)", "cs(User-Agent)",
